...
The Express API is deployed via `artifacts/tourvision/server.js` (Vercel rewrites `/api/*` → `/server`, lazy-loads `api/serverless.mjs`). It is **not** a separate Railway service unless you choose that option below.
